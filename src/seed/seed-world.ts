...
/**
 * E2: Where's My Truck? — the deterministic demo world, as data plus one writer.
 *
 ...
 * BASE (the top of the hour the seed runs in), so the demo is identical in
 * shape on every run but always sits around "now".
 *
 * It lives under `src/` rather than in `prisma/seed.ts` because two callers
 * need it: `pnpm db:seed` (the CLI wrapper, which owns its own client) and
 * `POST /api/v1/simulation/reset`, which rewinds the running demo. There is no
 * second definition of t0 — reset and re-seed produce the same world.
 *
 * Re-running is safe: all domain rows are deleted first, in FK-safe order.
 */

/** The seed writes through whatever client it is handed — the CLI's or the app's. */
export type SeedClient = Pick<PrismaClient, '$transaction'> & {
  route: { count(): Promise<number> };
  dockDoor: { count(): Promise<number> };
  truck: { count(): Promise<number> };
  shipment: { count(): Promise<number> };
  appointment: { count(): Promise<number> };
  dockAssignment: { count(): Promise<number> };
  alert: { count(): Promise<number> };
  locationHistory: { count(): Promise<number> };
};

export interface SeedCounts {
  /** The instant every demo timestamp is an offset from. */
  baseTime: Date;
...
interface DockSeed {
  ...
  latitude: number;
  longitude: number;
  availableFromOffsetMinutes: number | null;
...
const docks: DockSeed[] = [
  ...
  },
  {
    ...
    latitude: 22.5857,
    longitude: 88.4096,
    availableFromOffsetMinutes: null,
    unavailableReason: null,
  },
  {
    id: 'D3',
...
export async function seedWorld(prisma: SeedClient): Promise<SeedCounts> {
// ---------------------------------------------------------------------------
// Deterministic clock
// ---------------------------------------------------------------------------
...
const minutes = (n: number): Date => new Date(BASE.getTime() + n * 60_000);
    ...
    async (tx) => {
      ...
      await tx.dockDoor.createMany({
        data: docks.map((dock) => ({
          ...
          availableFrom:
            dock.availableFromOffsetMinutes === null
              ? null
              : minutes(dock.availableFromOffsetMinutes),
  ...
  logger.info(`Seed complete (base time ${BASE.toISOString()})`);

  return { baseTime: BASE, counts };
}
